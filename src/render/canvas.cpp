...
// Skia headers — only included in .cpp, never in public headers.
...
const SkPaint& as_sk_paint(const Paint& p) {
    return *static_cast<const SkPaint*>(p.native_handle());
}
...
void Canvas::draw_points(std::span<const PointF> pts, const Paint& p) {
    ...
    sk(sk_canvas_)->drawPoints(SkCanvas::kPoints_PointMode,
                               sk_pts.size(), sk_pts.data(), as_sk_paint(p));
}
...
void Canvas::draw_image(const Image& img, float x, float y, const Paint* p) {
    ...
    const SkPaint* sp = p ? static_cast<const SkPaint*>(p->native_handle()) : nullptr;
    sk(sk_canvas_)->drawImage(ref, x, y, SkSamplingOptions(), sp);
}
void Canvas::draw_image(const Image& img, RectF dst, const Paint* p) {
    draw_image(img, RectF{0, 0, float(img.width()), float(img.height())}, dst,
...
void Canvas::draw_text(std::string_view text, RectF bounds,
    ...
    float y = bounds.y;
    switch (baseline) {
    case TextBaseline::Top:        y = bounds.y - fm.fAscent; break;
    case TextBaseline::Middle:     y = bounds.y + bounds.h * 0.5f
                                       + (-fm.fAscent + fm.fDescent) * 0.5f
                                       - fm.fDescent; break;
    case TextBaseline::Bottom:     y = bounds.bottom() - fm.fDescent; break;
    case TextBaseline::Alphabetic: y = bounds.y; break;
    }
